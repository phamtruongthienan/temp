...
class HomeController extends Controller
{
    ...
    public function homepage_index(Request $request)
    {
        try {
            ...
            $promotion_top = $this->instance->getPromotion(null, 4, 3);
    ...
    public function homepage_school(Request $request)
    {
        try {
            if($request->has('sorts')) {
                ...
                $schools = $this->instance->getSchool(null, config('constant.pagination'), $sort);
            } else {
    ...
    public function homepage_edit_user(Request $request) 
    {
        $rules = array(
        ...
        $validator = Validator::make(Input::all(), $rules);
        if ($validator->fails()) {
            return "valiabort(404)";
            // return redirect('/');
        } else {
            try {
                    $promotion = $this->instance->postEditUser($request);
                    if($promotion==true) {
                        return "thành công edit user" ;//??wwith 
                    } else {
                        return abort(404);
                    }
            } catch (\Exception $e) {
    ...
    public function homepage_edit_child(Request $request) 
    {
        $rules = array(
            'idChild' => 'required|max:2',
            // 'gender' => 'required',
            // 'birthday'  => 'required|Date',
            // 'phone' => 'required|integer|max:9',
        );
        ...
        if ($validator->fails()) {
            ...
            try {
                    $promotion = $this->instance->postEditChild($request);        
                    if($promotion==true) {
                        return "thành công edit_child" ;//??wwith 
                    } else {
                        return abort(404);
                    }
            } catch (\Exception $e) {
    ...
    public function homepage_add_child(Request $request) 
    {
        $rules = array(
            // 'idChild' => 'required|max:2',
            // 'gender' => 'required',
        ...
        if ($validator->fails()) {
            ...
            try {
                    $promotion = $this->instance->postAddChild($request);        
                    if($promotion==true) {
                        return "thành công add_child" ;//??wwith 
                    } else {
                        return abort(404);
                    }
            } catch (\Exception $e) {
